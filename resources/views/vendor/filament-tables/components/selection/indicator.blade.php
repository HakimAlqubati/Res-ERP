...
<div x-cloak
    ...
    <div class="flex gap-x-3">
        <x-filament::loading-indicator x-show="isLoading" class="h-5 w-5 text-gray-400 dark:text-gray-500" />
        ...
        <span
            ...
            "
            class="text-sm font-medium leading-6 text-gray-700 dark:text-gray-200"></span>

        {{-- This condition checks if the current page has our custom PHP function --}}
        @if (method_exists($this, 'getRecordsWithDetails'))
            {{-- The JavaScript code you already added goes inside the 'if' --}}
            <div class="border-s border-gray-200 ps-3 dark:border-white/10" x-data="{
                recordsMap: @js($this->getRecordsWithDetails()),
                totalSum: 0,
                init() {
                    this.$watch('selectedRecords', (newSelected) => {
                        let sum = 0;
                        newSelected.forEach((id) => {
                            if (this.recordsMap[id] !== undefined) {
                                sum += this.recordsMap[id];
                            }
                        });
                        this.totalSum = sum;
                    });
                ...
                <span class="text-sm font-medium text-gray-700 dark:text-gray-200">
                    (  Products Count: <span x-text="totalSum"></span>)
                </span>
            </div>
        @endif
    ...
    <div class="flex gap-x-3">
        ...
        <div class="flex gap-x-3">
            <x-filament::link color="primary" tag="button" :x-on:click="$selectAllRecordsAction" :x-show="$selectCurrentPageOnly
                ? '! areRecordsSelected(getRecordsOnPage())'
                : $allSelectableRecordsCount . ' !== ' . $selectedRecordsPropertyName . '.length'"
