<x-filament-panels::page>
    ...
    @php
        ...
        $displayName = $employee
            ? $employee->name
            : ($branches->isNotEmpty() ? ($branches->count() > 2 ? __('Multiple Branches') : $branches->pluck('name')->join(' & ') . ' - ' . __('Branch')) : __('All Employees'));
    @endphp
    ...
    <table class="w-full text-sm text-left pretty" id="report-table">
        <thead class="fixed-header" style="top:64px;">
                    ...
                    <div style="display: flex; align-items: center; justify-content: space-between; gap: 16px;">
                        ...
                        <div style="display: flex; align-items: center; justify-content: center; gap: 10px; flex: 1;">
                            ...
                            <div>
                                ...
                                <span class="forecast-badge">{{ __('Forecast') }}</span>
                            </div>
                        </div>
            ...
            <tr>
                ...
                <th style="width: 30%; text-align: center;">{{ __('Amount (Estimated)') }}</th>
            </tr>
        </thead>
    ...
    <div style="text-align: center; padding: 40px;">
        <h1 style="font-size: 1.5rem; color: #555;">{{ __('Please select an Employee or Branch and a Period') }}</h1>
    </div>
    @endif
    ...
    <script>
        function exportToExcel() {
            ...
            wb['!cols'] = [{ wch: 50 }, { wch: 20 }];

            var workbook = XLSX.utils.book_new();
            XLSX.utils.book_append_sheet(workbook, wb, "Pre-Payroll Deductions");

            var reportTitle = "{{ preg_replace('/[^A-Za-z0-9_\\-]/', '_', $reportData['report_title'] ?? 'Report') }}";
            var fromDate = "{{ $reportData['from_date'] ?? '' }}";
            XLSX.writeFile(workbook, "PrePayroll_Deductions_" + reportTitle + "_" + fromDate + ".xlsx");
        }
    </script>
</x-filament-panels::page>
