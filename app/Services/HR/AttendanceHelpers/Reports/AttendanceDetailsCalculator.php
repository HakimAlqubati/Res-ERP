...
class AttendanceDetailsCalculator
{
    ...
    /**
     * Calculate detailed breakdown of attendance for the details modal (Original).
    ...
    /**
     * Calculate detailed breakdown of attendance with branches for the details modal.
     *
     * @param  array  $data  Raw attendance details (each with 'check_type', 'check_time', 'period_id', 'branch')
     * @return array
     */
    public static function calculateDetailedBreakdownWithBranches(array $data): array
    {
        $attendances = [];
        $totalMinutes = 0;
        ...
        foreach ($data as $detail) {
            $formattedTime = isset($detail['check_time']) ? Carbon::parse($detail['check_time'])->format('H:i') : null;
            $branchName = $detail['branch']['name'] ?? $detail['branch_name'] ?? '-';

            if ($detail['check_type'] === 'checkin') {
                $attendances[$detail['period_id']]['checkins'][] = $formattedTime;
                $attendances[$detail['period_id']]['checkin_branches'][] = $branchName;
            } elseif ($detail['check_type'] === 'checkout') {
                $attendances[$detail['period_id']]['checkouts'][] = $formattedTime;
                $attendances[$detail['period_id']]['checkout_branches'][] = $branchName;
            }
        }
        ...
        foreach ($attendances as $index => $attendance) {
            ...
            for ($i = 0; $i < $maxRows; $i++) {
                $checkin = $attendance['checkins'][$i] ?? null;
                $checkout = $attendance['checkouts'][$i] ?? null;
                $inBranch = $attendance['checkin_branches'][$i] ?? null;
                $outBranch = $attendance['checkout_branches'][$i] ?? null;

                // Build branch label
                $branchLabel = '-';
                if ($inBranch && $outBranch) {
                    $branchLabel = ($inBranch === $outBranch) ? $inBranch : "{$inBranch} ➔ {$outBranch}";
                } elseif ($inBranch) {
                    $branchLabel = $inBranch;
                } elseif ($outBranch) {
                    $branchLabel = $outBranch;
                }
                $attendances[$index]['branch_name'][$i] = $branchLabel;
