...
class SalaryCalculatorService implements SalaryCalculatorInterface
{
    ...
    public function calculate(
        ...
        if (
            ...
            // Allocate the fixed 30 salary days cumulatively across every branch
            // segment. Calculating only the last segment proportionally allowed a
            // 27 + 4 split in a 31-day month to be paid as 31 days.
            $daysBeforeSegment = $periodStart->day - 1;
            $daysThroughSegment = $periodEnd->day;

            $payableDays = max(0,
                (int) round($daysThroughSegment / $monthDays * $rateWorkingDays)
                - (int) round($daysBeforeSegment / $monthDays * $rateWorkingDays)
            );
