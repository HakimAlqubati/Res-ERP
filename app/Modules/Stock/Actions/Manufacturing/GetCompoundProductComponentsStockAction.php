...
final class GetCompoundProductComponentsStockAction
{
     ...
     */
    public function execute(int $compoundProductId, int $storeId): Collection
    {
        // 1. Fetch components of the compound product
        $components = ProductItem::with(['product', 'unit'])
            ->where('parent_product_id', $compoundProductId)
            ->get();
        ...
        return $components->map(function (ProductItem $component) use ($balances) {
